...
### 56. 误餐补助无票费用记录的 `F_ZSC_DEF_001` 控制维度强校验与二次联动自愈机制 (v4.75.0)

**时间**：2026-09-17  
**严重级别**：🔴 P1 — 导致生成报销单草稿时被刚性拦截：“第X条：费用记录存在禁止消息，请修改后关联”  
**现象**：  
  出差费用报销单管理工作台在生成报销单草稿时，如果为正社员自动补齐无发票的误餐补助明细，调用 `createBillDataAndTemplateByExpenseIdList` 接口会被系统强行拦截，提示 `费用记录存在禁止消息，请修改后关联`。在系统 UI 打开该误餐补助编辑抽屉时，红色高亮提示：“住宿城市类型必填”。  
**底层根因链条**：  
  1. 元年云系统费用类型 `0356c51e601de1653e55bb00bc610000`（误餐补助）内部配置了定额补贴标准模型 `035a49b60c8345af7f1906ec05cc0001`。  
  2. 该模型强制要求控制维度 `F_ZSC_DEF_001`（住宿城市类型：北上广深/其他）必须非空；而该控制维度在单据初次新建态（`operationType: 'ADD'`）时，即使调用了 `fieldValueChange` 联动出差城市，后端也不会直接将 `F_ZSC_DEF_001` 落盘，留下了残缺状态。  
  3. 用户在 UI 编辑界面手动切换选中一次出差城市（天津），触发的是针对已持久化实体的 `fieldValueChange`（`UPDATE` 模式），后端才会将推导出的 `F_ZSC_DEF_001`（`035a403ff62345af7f1906ec05cc0000` 境内-其他）写入并消除错误。  
**终极解法 (Self-Healing on Save)**：  
  1. 在 `createMealAllowanceExpenseRecordApi` 中，先按正规 4 步规则联动（业务日 ➔ 开始时间 ➔ 城市 ➔ 结束时间）完成初次持久化落盘；  
  2. 随后主动调取该实体的校验结果，若仍检出 `F_ZSC_DEF_001` 缺失，立即自动执行一次以当前城市维表为入参的 `fieldValueChange`，并将推导出的标准对象通过 `UPDATE` 再次持久化，彻底清除禁止消息。  
**修复版本**：`v4.75.0`

---

### 57. 出差申请台账主键 (`machineAccountId`) 与单据实体主键 (`billMainId`) 区别及费用行 `F_CCSQDH` 双向绑定规约 (v4.75.0)

**时间**：2026-09-17  
**严重级别**：🟠 P2 — 导致生成的报销单在官方系统打开时，表头申请单脱节为空，展开区各费用的预算归属也要重新选择  
**现象**：  
  自动生成的报销单草稿在官方编辑页（`#billWrite`）打开时，表头的出差申请单显示未关联，展开区中各项费用的预算归属与台账绑定也全部为空。  
**底层根因链条**：  
  1. 报销单表头字段 `F_CCSQD` 是 `MACHINE_ACCOUNT`（台账关联）对象，其内部 `machineAccountId` 必须是出差台账表的行主键（如 `04883e91bdc3...`），而非出差申请单单据主表实体 ID（`billMainId: 04882ac23e39...`）；若传入实体 ID，官方 UI 检索台账池查不到对应记录，直接渲染为空。  
  2. 官方表单在展开费用分摊明细时，严格依赖费用明细主行（`T_BILL_AREA_EXPENSE_CLAIM`）中的 `F_CCSQDH`（申请单号），以及预算子行中的 `F_WFBM_01`、`APPLY_BILL_CODE_EXT` 将费用分摊行与单据头的申请单台账池咬合绑定；缺失这些字段会导致分摊行无法带出核准的申请单项目。  
**终极解法**：  
  1. 申请单列表查询时从 `billRequestMachineAccountDataVOS` 树中穿透提取真实的 `machineAccountId`，回写 `F_CCSQD`；  
  2. 生成报销单分摊行时，向每笔费用主行补齐 `F_CCSQDH: SC单号`，向预算分摊行注入 `F_WFBM_01`、`APPLY_BILL_CODE_EXT` 以及 `F_KYJE`，并纳入极速克隆字段集，确保展开区的所有费用行原生具备申请单绑定。  
**修复版本**：`v4.75.0`
